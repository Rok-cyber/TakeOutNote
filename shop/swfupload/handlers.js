...
function AddList(src) {
	...
	new_list.className = "listName";
	new_list.onmouseover = function () { new_list.className = "listName2"; return false; };
	new_list.onmouseout	 = function () { new_list.className = "listName"; return false; };
	new_list.innerHTML = "<div style='float:left'><img src='"+r_data[0]+"' align='absmiddle'> "+r_data[1]+"</div><div class='listSize' style='float:right;'>"+r_data[2]+" <a href='#' onclick='DelList(\""+r_data[1]+"\");return false;'><img src='/swfupload/icon/del.gif' align='absmiddle' border='0'></a></div>";
...
function removeList(element, opacity,size) {
	...
	if (opacity > 0) {		
		...
		var cnts=document.getElementById("fileList").childNodes.length;
		if(document.selection) var cnts=document.getElementById("fileList").childNodes.length;
...
function initList(src) {
	for(i=0,cnts=src.length;i<cnts;i++) {
		...
		new_list.id = "list_"+r_data[1];
		new_list.className = "listName";
		new_list.onmouseover = function () { this.className = "listName2"; return false; };
		new_list.onmouseout	 = function () { this.className = "listName"; return false; };
		new_list.innerHTML = "<div style='float:left'><img src='"+r_data[0]+"' align='absmiddle'> "+r_data[1]+"</div><div class='listSize' style='float:right;'>"+r_data[2]+" <a href='#' onclick='DelList(\""+r_data[1]+"\");return false;'><img src='/swfupload/icon/del.gif' align='absmiddle' border='0'></a></div>";
...
function DelList(name) {
	...
	aObj.getHttpRequest("/swfupload/swf_upload.php?mode=del&file="+name+"&tmp_dir="+tmp_dir, "resultList","data");
	//aObj.getHttpRequest("/swfupload/tmp.php","resultList","data");
}
