...
if($mode=="summ") {
	$new_time = (time()-86400);
	$where = " && (signdate > '{$new_time}' || moddate > '{$new_time}')";
}
...
while($data = $mysql->fetch_array()){
	...
	if($data['moddate']>0) $MDATE	= date("Y-m-d H:i:s",$data['moddate']);
	else $MDATE	= date("Y-m-d H:i:s",$data['signdate']);

	if($mode=="summ") {
		if($data['signdate']>$new_time) $TYPE = 'C';
		else $TYPE = 'U';
	}
	...
	switch($mode) {
		case "summ" :
			echo "{$UID}<!>{$TYPE}<!>{$NAME}<!>{$PRICE}<!>{$LINK}<!>{$IMAGE}<!>{$ocate1}<!>{$ocate2}<!>{$ocate3}<!>{$ocate4}<!>{$cate1}<!>{$cate2}<!>{$cate3}<!>{$cate4}<!>{$data['model']}<!>{$BRAND}<!>{$data['comp']}<!>{$data['made']}<!>{$DATE}<!>{$CARR}<!>{$EVENT}<!>{$COUPON}<!><!>{$RESE}<!><!><!><!>{$MDATE}\r\n";
		break;
		
		default :
			echo "{$UID}<!>C<!>{$NAME}<!>{$PRICE}<!>{$LINK}<!>{$IMAGE}<!>{$ocate1}<!>{$ocate2}<!>{$ocate3}<!>{$ocate4}<!>{$cate1}<!>{$cate2}<!>{$cate3}<!>{$cate4}<!>{$data['model']}<!>{$BRAND}<!>{$data['comp']}<!>{$data['made']}<!>{$DATE}<!>{$CARR}<!>{$EVENT}<!>{$COUPON}<!><!>{$RESE}<!><!><!><!>{$MDATE}\r\n";
		break;
	}
}
