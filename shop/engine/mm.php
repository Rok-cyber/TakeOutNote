...
$row = explode("|",$row);
if($row[6]!=1) alert("야후엔진페이지 미사용 상태 입니다.","close");
$exword = $row[7];
$fname = "mm";
...
echo "<p>TOTAL:".$mysql->affected_rows()."\r\n";
...
while($data = $mysql->fetch_array()){
	...
	$LK		= str_replace("index.php?","",$gData['link']);
	...
	$DATE	= date("Ymd",$data['signdate']);

	if($data['event']>0) $ENAME = $EVENT_NAME[$data['event']];
	else $ENAME = '';
	...
	if(substr($data['cate'],6,3)!='000') {
		$cate_arr[$data['cate']] = str_replace("/","&",$cate_arr[$data['cate']]);
		$cate3 = "/".$cate_arr[$data['cate']];
	} 
	else $cate3 = '';
	if(substr($data['cate'],3,3)!='000') {
		$cate_arr[substr($data['cate'],0,6)."000"] = str_replace("/","&",$cate_arr[substr($data['cate'],0,6)."000"]);
		$cate2 = "/".$cate_arr[substr($data['cate'],0,6)."000"];
	}
	else $cate2 = '';

	$cate_arr[substr($data['cate'],0,3)."000000"] = str_replace("/","&",$cate_arr[substr($data['cate'],0,3)."000000"]);
	$cate1 = $cate_arr[substr($data['cate'],0,3)."000000"];
	...
	echo "<p>{$UID}[^]{$NAME}[^]{$LINK}[^]{$PRICE}[^]{$cate1}{$cate2}{$cate3}[^]{$data['comp']}[^]{$IMAGE}[^]{$CARR}[^]{$COUPON}[^][^][^]\r\n";
}
