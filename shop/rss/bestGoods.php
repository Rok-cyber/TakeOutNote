...
if($cate) {
	...
	$TITLE = stripslashes($mysql->get_one($sql));
	$TLINK = "/{$ShopPath}{$Main}?channel=main2&cate={$cate}";
}
else {
	$TITLE = '';
	$TLINK = "/{$ShopPath}".$Main;
...
if($cate) {
	...
	switch($cate_len) {
		...
		break;	
		case "9" : 
			$where .= " && SUBSTRING(cate,1,9) = '{$cate}' ";		    			
		break;
		default : $where .= " && cate = '{$cate}' ";		    			
    }	
	$cstring .= "&cate={$cate}";
}
...
/**************************** GOODS LIST**************************/
$sql = "SELECT uid,cate,number,name,price,price_ment,image3,image4,icon,comp,reserve,c_cnt,event FROM mall_goods WHERE s_qty !='3' && type='A' {$where} ORDER BY o_cnt DESC LIMIT 50";
$mysql->query($sql);
