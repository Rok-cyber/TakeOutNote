...
function skinOnScroll() {  //onscroll시
	if(document.getElementById('rightBox')) {

		function moveLayer(){		
			tObj = document.getElementById('rightBox');
			if(tObj==null) return;
			tTo  = (document.body.scrollTop || document.documentElement.scrollTop);
			tFrom= parseInt(tObj.style.top);

			if(typeof(rBoxStart)=="undefined") rBoxStart = 0;
			if(typeof(rBoxOT)!="undefined") {				
				if(tTo < rBoxOT) { tTo = 0; rBoxST = 0; }
				else if(rBoxOT+tFrom>tTo) return;
				//else if(rBoxST==0) rBoxST = tTo;				
			}
			
			tTo = tTo + rBoxStart - rBoxST;
...
function ckOption(obj) {
	...
	if(document.getElementById('totals')) {
		...
		if(tmps[1] && tmps[1]>0) {
			total = (eval(document.goodsForm.p_price.value) + eval(tmps[1]))* eval(document.goodsForm.p_qty.value);
			document.goodsForm.p_op.value = tmps[1];
			document.getElementById('totals').innerHTML = number_format(total)+'원';
		}
		else {
			total = eval(document.goodsForm.p_price.value) * eval(document.goodsForm.p_qty.value);
			document.goodsForm.p_op.value = 0;
			document.getElementById('totals').innerHTML = number_format(total)+'원';
		}
	}
	...
	if(document.getElementById('prices')) {
		tmps = obj.value.split('|');
		if(tmps[1] && tmps[1]>0) {
			total = (eval(document.goodsForm.p_price.value) + eval(tmps[1]));
			document.goodsForm.p_op.value = tmps[1];
			document.getElementById('prices').innerHTML = number_format(total)+'원';
		}
		else {
			total = eval(document.goodsForm.p_price.value);	
			document.goodsForm.p_op.value = 0;
			document.getElementById('prices').innerHTML = number_format(total)+'원';
		}
	}
...
function ckTotal(){
	if(document.getElementById('totals')) {
		total = (eval(document.goodsForm.p_price2.value) + eval(document.goodsForm.p_op.value))* eval(document.goodsForm.p_qty.value);
		document.getElementById('totals').innerHTML = number_format(total)+'원';
	}
}	
...
function ckGoods(type,ck){	
	...
	if(type=='cooper') {		
		...
		if(f.ckLogin.value=='N') {			
			pLightBox.show(paths+'php/plogin.php?type=vorder','iframe',450,360,'■ 로그인','20');
		}
		else f.submit();
		return false;
	}
...
function rtnConfirmValue(){	
	...
	if(f.ckLogin.value=='N') {
		pLightBox.show(paths+'php/plogin.php?type=vorder'+pt_forms,'iframe',450,360,'■ 로그인','20');
	}
	else f.submit();
}
...
function ckGoods2(type,num){	
	...
	if(type=="order") {
		...
		else if(f.ckLogin.value=='N') pLightBox.show('plogin.php?type=vorder&num='+num,'iframe',450,360,'■ 로그인','20');
		else f.submit();
		return false;
	}
	else {
...
function rtnConfirm3Value(){	
	if(messageBox.getValue()=='확인') {
		pLightBox.show('php/plogin.php?type='+tmpType,'iframe',450,360,'■ 로그인','20');
		tmpType = '';
	}	
}
...
function zipcode(cnt) {	
	pLightBox.show('php/pzipcode.php?fname=join&ocnt='+cnt,'iframe',420,220,'■ 우편번호 찾기','20');
}		

function zipcode2() {
	pLightBox.show('php/pzipcode.php','iframe',420,220,'■ 우편번호 찾기','20');
}
...
function openQview(num) {
	pLightBox.show('php/pquick_view.php?uid='+num,'iframe','730','420','■ Quick View','20');	
}

function openBimg(num) {
	pLightBox.show('php/pbig_image.php?uid='+num,'iframe','730','560','■ 큰이미지 보기','20');	
}
...
function scateShow(type) {
	...
	document.getElementById("scateImg").src = shop_skin + "/img/main/cate_view_" + type + ".gif";	
}
...
function openCM(num,img,cimg,show) {  	
	...
	document.getElementById("font"+(parseInt(num)+1)).style.color = cateColor;
...
function closeCM(num,hide) { 		
	...
	document.getElementById("font"+(parseInt(num)+1)).style.color = '';
...
function delGoodsOk1(data) {
	if(data['item']=='true' && data['uid']) {			
		...
		if(rCartBoxDiv.tcnt==0) {
			...
			tag.innerHTML = '<li style="height:120px;"></li><li class="small gray" style="line-height:140%">장바구니 상품이 <br>없습니다.</li>';
			obj.appendChild(tag);		
			rCartBoxDiv.count = 1;
...
function ajaxPaging(name,id,rtnId,total_record,page_record_num,page_link_num,tFocus,obj1,obj2) {
	...
	this.printPage = function (type,imgPath,separ,pt_fb) {
		...
		if(this.block > 1) {
			switch(type) {
				case "img" :
					paging	= '<a href="#" onclick="return '+this.name+'.resultPage(\'1\',1);" onfocus="this.blur();"><img src="'+imgPath+'/btn_first.gif" border="0" style="vertical-align:middle;" alt="첫페이지" title="첫페이지" /></a>&nbsp;';
					paging += '<a href="#" onclick="return '+this.name+'.resultPage(\''+this.prev_page+'\',1);" onfocus="this.blur();"><img src="'+imgPath+'/btn_prev.gif" border="0"  style="vertical-align:middle;" alt="이전 목록" title="이전 목록" /></a>&nbsp;';
				break;
			
				default :
					paging  = '<a href="#" onclick="return '+this.name+'.resultPage(\''+this.prev_page+'\',1);" onfocus="this.blur();"><span id="prevPage" title="이전 목록">이전</span></a>';
					paging += '<span class="numbox">';
					paging += '<a href="#" onclick="return '+this.name+'.resultPage(\'1\',1);" onfocus="this.blur();"><span class="num2" title="첫 페이지">1</span></a><span class="num">...</span>';
			}		
		}
		else {
			switch(type) {
		...
		if(this.block<this.total_block) {
			switch(type) {
				case "img" :
					paging += '&nbsp;<a href="#" onclick="return '+this.name+'.resultPage(\''+this.next_page+'\',1);" onfocus="this.blur();"><img src="'+this.imgPath+'/btn_next.gif" border="0" style="vertical-align:middle;" alt="다음 목록" title="다음 목록" /></a>&nbsp;';
				    paging += '<a href="#" onclick="return '+this.name+'.resultPage(\''+this.total_page+'\',1);" onfocus="this.blur();"><img src="'+this.imgPath+'/btn_last.gif" border="0"  style="vertical-align:middle;" alt="마지막 페이지" title="마지막 페이지" /></a>';
				break;

				default :
					paging += '<span class="num2">...</span><a href="#" onclick="return '+this.name+'.resultPage(\''+this.total_page+'\',1);" onfocus="this.blur();"><span class="num" title="마지막 페이지">'+this.total_page+'</span></a>&nbsp;';
					paging += '<a href="#" onclick="return '+this.name+'.resultPage(\''+this.next_page+'\',1);" onfocus="this.blur();"><span id="nextPage" title="다음 목록">다음</span></a>';
			}			
		}			
...
function orderPost(ckLog,skin){
	...
	if(ckLog=="N") {
		pLightBox.show('php/plogin.php','iframe',450,360,'■ 로그인','20');
	}
...
function msInfo(obj,num) {
	if(obj.checked) {
		...
		f.ckMs2.checked = false;
		f.ckMs3.checked = false;
...
function ckOrder() {  
	...
	if(typeof(f.agree1)!='undefined') {
		if(f.agree1.checked!=true) {
			alert("이용약관에 동의 하셔야만 상품을 구매하실 수 있습니다.");
			return false;
		}
		if(f.agree2.checked!=true) {
			alert(" 개인정보취급방침에 동의 하셔야만 상품을 구매하실 수 있습니다.");
			return false;
		}		
	}
