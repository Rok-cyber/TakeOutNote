...
$tpl->define("main","./search_sms_addr.html");
$tpl->scan_area("main");
$tpl->parse("is_man1");
...
/*********************************** QUERY **********************************/
    $query = "SELECT groups, name, cell FROM {$code} WHERE uid != '0' {$where} ORDER BY {$order} LIMIT $Pstart,$limit";
    $mysql->query($query);	
/*********************************** QUERY  ***********************************/
...
/*********************************** LOOP  ***********************************/
	while ($row=$mysql->fetch_array()){
		...
		$LIST3 = stripslashes($row['name']);
		$LIST4 = stripslashes($row['cell']);		
		$DEL	= "<input type='checkbox' value='{$LIST4}' name='item[]' onfocus='blur();'>";
