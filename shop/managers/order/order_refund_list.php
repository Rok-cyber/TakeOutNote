...
if($status) {
	...
	if($status=='X') $where .= " (status='X' && status2='C')";
	else $where .= " (status='Z' && status2='B')";
}
else $where = "(status='Z' && status2='B') || (status='X' && status2='C')";
...
$sql = "SELECT COUNT(*) FROM mall_order_change WHERE {$where}";
...
$tpl->define("main","./order_refund_list.html");
$tpl->scan_area("main");
$tpl->parse("is_man1");
...
if($total_record > 0) {
	...
	/*********************************** QUERY **********************************/
    $query = "SELECT * FROM mall_order_change WHERE {$where} ORDER BY uid DESC LIMIT {$Pstart},{$record_num}";
	$mysql->query($query);		
	/*********************************** QUERY  ***********************************/
