...
if($field && $word) {
	...
	if($field=="multi") $where .= "&& (INSTR(name,'{$word}') || INSTR(model,'{$word}') || INSTR(tag,'{$word}') || INSTR(uid,'{$word}')";
	else $where .= "&& (INSTR({$field},'{$word}') ";

	if(eregi("'",$word)) {
		$word2 = addslashes($word);
		if($field=="multi") $where .= "|| INSTR(name,'{$word2}') || INSTR(model,'{$word2}') || INSTR(tag,'{$word2}') || INSTR(uid,'{$word2}'))";
		else $where .= "|| INSTR({$field},'{$word}')) ";
	}
	else $where .= ")";
...
/*********************************** QUERY **********************************/
	...
	$query .= $MTlist[$i]." FROM {$code} WHERE uid != '0' {$where} ORDER BY {$order} LIMIT $Pstart,$limit";
    $mysql->query($query);	
/*********************************** QUERY  ***********************************/
...
/*********************************** LOOP  ***********************************/
	while ($row=$mysql->fetch_array()){
		...
		$UID	= $row['uid'];
		$MLINK	= "goods_write.php?mode=modify&uid={$UID}{$addstring}";
		...
		switch($LIST5){
			case "1" : $LIST5 = "무제한";
			break;
			case "2" : $LIST5 = "<font class='small' style='color:#3366CC'>품절</font>";
			break;
			case "3" : $LIST5 = "<font class='small' style='color:#3366CC'>상품숨김</font>";
			break;
			case "4" : 
				$LIST5 = "<input type='text' name='qty_{$UID}' size='3' class='ta2 eng' onkeydown='onlyNum(this);' onkeyup='onlyNum(this);' maxlength='5' value='{$LIST6}'>";
			break;
		}
		if($row['type']=='B') $LIST5 = "<font class='small' style='color:#3366CC'>분류<br />상품숨김</font>";
		
		$LIST13 = "<select name='sequence_{$UID}'><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option></select><script>document.selectForm.sequence_{$UID}.value='{$row['sequence']}';</script>";
		...
		if($LIST16 && $LIST16!=0) {
			...
			$LIST16 = "{$tmp_empty}<font class='small blue'>브랜드</font> : <a href='{$_SERVER['PHP_SELF']}?brands={$LIST16}'>".$brand_arr[$LIST16]."</a>";
		}
		else $LIST16 = '';
