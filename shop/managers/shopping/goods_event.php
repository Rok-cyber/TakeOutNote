...
while($row=$mysql->fetch_array()){
	if(!$event) $event = $row['uid'];
	if($row['uid'] == $event) $sec = 'selected';
	else $sec='';
	...
	$EVENT .= "<option value='{$row['uid']}' {$sec}>{$row['name']}</option>\n";
}	
if(!$EVENT) $EVENT = "<option value=''>진행중없음</option>";
...
if($EVENT=="<option value=''>진행중없음</option>") $total_record = 0;
...
$tpl->define("main","./goods_event.html");
$tpl->scan_area("main");
$tpl->parse("is_man1");
