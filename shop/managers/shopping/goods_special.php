...
while($row=$mysql->fetch_array()){
	if(!$special) $special = $row['uid'];
	if($row['uid'] == $special) $sec = 'selected';
	else $sec='';
	$row['name'] = stripslashes($row['name']);
	$row['name'] = str_replace("\"","&#034;",$row['name']);
	$row['name'] = str_replace("'","&#039;",$row['name']);
	$SPECIAL .= "<option value='{$row['uid']}' {$sec}>{$row['name']}</option>\n";	
}	

if($special) $where .= " && INSTR(special,',{$special},')";
else $SPECIAL = "<option value=''>기회전없음</option>";
...
$tpl->define("main","./goods_special.html");
$tpl->scan_area("main");
$tpl->parse("is_man1");
