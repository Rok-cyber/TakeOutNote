	...
	if($row['cate_sub']==1) {
		...
		while($row2=$mysql->fetch_array(2)){
			...
			if($row2['cate_sub']==1) {
				...
				$tmps2.= ",['{$row2[cate]}'";	
				$sql3 = "SELECT cate,cate_name FROM mall_cate WHERE cate_dep = '3' AND cate_parent = '{$row2[cate]}' ORDER BY number ASC";
				$mysql->query3($sql3);
...
$disps		= isset($_GET['disps']) ? $_GET['disps'] : $_POST['disps'];
...
if($seccate) {
	if(substr($seccate,3,9)=='000000000') {
		...
		$cate2 = " ==== 2차분류 ==== ";
		$where .= "&& SUBSTRING(display,3,1)='{$disps}'";
		$order = "o_num2";
	} 
	...
	$where .= "&& SUBSTRING(display,1,1)='{$disps}'";
	$order = "o_num1";
}
