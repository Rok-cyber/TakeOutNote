<?
	...
	function zbDB_getDataList($tableName,$table="") {
		global $connect;
		$result = mysql_query("show fields from $tableName", $connect) or die(mysql_error());
		while($data=mysql_fetch_array($result)) {
			$field .= $data["Field"].",";
		...
		while($data=mysql_fetch_array($result)) {
		  if(!$table || ($table && $table == $data[name])){ 
			unset($str);
	...
	function pDB_manager($table) {
		echo "\n#\n# 'pboard_manager' data \n#\n";
		echo "\n";
		zbDB_getDataList("pboard_manager",$table);
		echo "\n# End of pboard_manager Dump\n";
		flush();
	}

	function zbDB_All_down($dbname) {
		global $connect;
		$result = mysql_query("show table status from $dbname",$connect) or die(mysql_error());
		$i=0;
