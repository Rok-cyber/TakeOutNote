...
$sql = "SELECT SUM(cnts) as total FROM pcount_agent WHERE type='K'";
if($TOTAL = $mysql->get_one($sql)) {

	$sql = "SELECT count(*) FROM pcount_agent WHERE type='K'";
	...
	$sql = "SELECT cnts FROM pcount_agent WHERE type='K' ORDER BY cnts DESC LIMIT 1";
	if(!$MAX = $mysql->get_one($sql)) $MAX=0;

	$sql = "SELECT * FROM pcount_agent WHERE type='K' ORDER BY cnts DESC LIMIT $Pstart,$record_num";
	$mysql->query($sql);
	
	while($data=$mysql->fetch_array()){
		$TTL2 = "<font class='small'>".urldecode($data[content])."</font>";
    ...
    if($total_record>$record_num) {
        ...
        $pg->addQueryString("?channel=keyword"); 
