...
$sql = "SELECT SUM(cnts) as total FROM pcount_agent WHERE type='B'";
if($TOTAL = $mysql->get_one($sql)) {
	$sql = "SELECT cnts FROM pcount_agent WHERE type='B' ORDER BY cnts DESC LIMIT 1";
	if(!$MAX = $mysql->get_one($sql)) $MAX=0;
	
	$NUM = 0;
	$sql = "SELECT * FROM pcount_agent WHERE type='B' ORDER BY cnts DESC";
	$mysql->query($sql);
