...
$sql = "SELECT SUM(cnts) as total FROM pcount_agent WHERE type='S'";
if($TOTAL = $mysql->get_one($sql)) {

	$sql = "SELECT count(*) FROM pcount_agent WHERE type='S'";
	...
	$sql = "SELECT cnts FROM pcount_agent WHERE type='S' ORDER BY cnts DESC LIMIT 1";
	if(!$MAX = $mysql->get_one($sql)) $MAX=0;

	$sql = "SELECT * FROM pcount_agent WHERE type='S' ORDER BY cnts DESC LIMIT $Pstart,$record_num";
	$mysql->query($sql);
