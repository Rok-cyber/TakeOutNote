...
$sql = "SELECT SUM(total) as total, SUM(total2) as total2 FROM pcount_list_affiliate WHERE uid!='' {$where}";
...
$sql = "SELECT SUM(total2) as t2 FROM pcount_list_affiliate WHERE uid!='' {$where} GROUP BY year ORDER BY t2 DESC LIMIT 1";
if(!$MAX = $mysql->get_one($sql)) $MAX=0;

for($i=$s_year;$i<=$year;$i++){
	$TTL2 = $i."<font class=small>년</font>";
	if($TOTAL !='0') {
		$sql = "SELECT SUM(total) as total, SUM(total2) as total2 FROM pcount_list_affiliate WHERE year = '{$i}' {$where}";
