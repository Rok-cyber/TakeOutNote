...
$sql = "SELECT SUM(total) as total, SUM(total2) as total2 FROM pcount_list WHERE year = '$year' && month = '$month'";
...
$sql = "SELECT total2 FROM pcount_list WHERE year = '$year' && month = '$month' ORDER BY total2 DESC LIMIT 1";
...
for($i=1;$i<=$maxdate;$i++){
	...
	if($TOTAL !='0') {
		$sql = "SELECT SUM(total) as total, SUM(total2) as total2 FROM pcount_list WHERE year = '$year'  && month = '$month' && day = '$i'";
