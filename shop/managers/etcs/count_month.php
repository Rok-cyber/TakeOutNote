...
$sql = "SELECT SUM(total) as total, SUM(total2) as total2 FROM pcount_list WHERE year = '$year'";
...
$sql = "SELECT SUM(total2) as t2 FROM pcount_list WHERE year = '{$year}' GROUP BY month ORDER BY t2 DESC LIMIT 1";
if(!$MAX = $mysql->get_one($sql)) $MAX=0;

for($i=1;$i<=12;$i++){
	$TTL2 = $i."<font class=small>월</font>";
	if($TOTAL !='0') {
		$sql = "SELECT SUM(total) as total, SUM(total2) as total2 FROM pcount_list WHERE year = '$year' && month = '$i'";
