...
$TSALES =  number_format($info['use_reserve'] + $info['use_cupon'] + $cashdc,$ckFloatCnt);
...
switch($info['pay_type']) {
	...
	case "C" :
		...
		if($info['pay_number'] && $info['pay_status']=='B') {
			...
			if($cash[2]=="LGDACOM") {
				...
				$authdata = md5($SHOP_ID.$tno.$SHOP_KEY);
				$tpl->parse("is_card_print_lgd");
			}
			else {
	...
	case "R" :
		...
		if($info['pay_number'] && $info['pay_status']=='B') {
			$tno = $info['pay_number'];
			if($cash[2]=="KCP")  $tpl->parse("is_rbank_print");
		}
		$tpl->parse("is_card");			
	break;
	case "V" :
