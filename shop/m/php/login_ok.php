...
$url	= !empty($_POST['url']) ? $_POST['url']:"http://".$_SERVER['HTTP_HOST']."/{$ShopPath}m/{$Main}";
...
$sql = "SELECT passwd,name,email,level,homepage,icon,auth FROM pboard_member WHERE id = '{$id}'";
...
if($_POST['auto_in']=='Y') $tm = time()+3600*24*365;
else $tm = 0;
...
switch($url) {
	...
	case "cart" :
		$url = "http://".$_SERVER['HTTP_HOST']."/{$ShopPath}m/{$Main}?channel=cart";			
	break;	

	case "order" :
		$url = "http://".$_SERVER['HTTP_HOST']."/{$ShopPath}m/{$Main}?channel=order_form";	
	break;	
	
	case "vorder" :
		$url = "http://".$_SERVER['HTTP_HOST']."/{$ShopPath}m/{$Main}?channel=order_form&direct=Y";	
	break;	
}
