...
switch($type) {	
	...
	case "corder" :
		...
		if($my_id) movePage("../{$Main}?channel=order_form&direct=Y");
		else movePage("../{$Main}?channel=login&type=corder");
	break;
	...
	case "mod": // 장바구니 수정모드
		...
		if($row['s_qty']==4 && $row['qty']<$p_qty) alert("물품재고량(현재:{$row['qty']}개)을 초과 했습니다!","back");
