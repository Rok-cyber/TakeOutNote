...
if($my_id) {
	...
	$sql = "SELECT * FROM mall_order_info WHERE id='{$my_id}' ORDER BY uid DESC LIMIT 1";
	if($last_order = $mysql->one_row($sql)){
		$NAME4 = stripslashes($last_order['name2']);
		$tel = explode(" - ",$last_order['tel2']);
		$TEL41 = $tel[0];
		$TEL42 = $tel[1];
		$TEL43 = $tel[2];
		$phone = explode(" - ",$last_order['hphone2']);
		$PHONE41 = $phone[0];
		$PHONE42 = $phone[1];
		$PHONE43 = $phone[2];
		$zip = explode(" - ",$last_order['zipcode']);
		$ZIP41 = $zip[0];
		$ZIP42 = $zip[1];
		$ADDR4 = stripslashes($last_order['address']);

		$tpl->parse("is_carrrecent");
	}
}
else {
	$TEL1 = "02";
...
if($TCNT==0) $tpl->parse('no_cart'); 
	...
	if($cash[2]=='KCP') {
		if($cash[1]=='1') { $tpl->parse("is_card1"); $cash_cnt++; }
		if($cash[17]=='1') { $tpl->parse("is_card2"); $cash_cnt++; }
		if($cash[18]=='1') { $tpl->parse("is_card3"); $cash_cnt++;}
		if($cash[19]=='1') { $tpl->parse("is_card4"); $cash_cnt++; }
	...
	else $RESERVE_USE2 = $RESERVE_USE = 0;
	$cash_cnt = round(100/$cash_cnt,1);
	/**************************** 결재 관련 *****************************/
