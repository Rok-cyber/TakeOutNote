...
if(function_exists('mt_rand')) $order_num = mt_rand(1000,9999);
else $order_num = rand(1,9).rand(0,9).rand(0,9).rand(0,9);
...
while($cart_list = $mysql->fetch_array()){
	...
	if($my_id !='guest') { //적립금 내역 저장
		...
		if($cart_list['p_reserve']>0) {
			$sql = "INSERT INTO mall_reserve (uid, id, subject, reserve, order_num, goods_num, status, signdate) VALUES ('','{$my_id}','{$subject}','".($cart_list['p_reserve']*$cart_list['p_qty'])."','{$order_num}','{$goods_num}','A','{$signdate}')";
			$mysql->query2($sql);
		}
    }	
...
if($my_id != 'guest' && $use_reserve >0){  //적립금 사용 저장
	 ...
	 $mysql->query($sql);
	 $subject = "상품구입 적립금 사용";
	 $sql = "INSERT INTO mall_reserve (uid, id, subject, reserve, order_num, goods_num, status, signdate) VALUES ('','{$my_id}','{$subject}','{$use_reserve}','{$order_num}','','C','{$signdate}')";
	 $mysql->query($sql);
}
...
switch($cash_type) {
	case "C" : case "R" : case "V" : case "H" :
		movePage("http://".$_SERVER['HTTP_HOST']."/{$ShopPath}{$Main}?channel=card_pay&amp;cash_type={$cash_type}&amp;order_num={$order_num}"); 
	break;
	...
	default : 
		if($cash_total==0) {
			...
			$signdate = date("Y-m-d H:i:s",time());
			$sql = "UPDATE mall_order_goods SET  order_status = 'B', status_date = '{$signdate}' WHERE order_num='{$order_num}'";
			$mysql->query($sql);
		}
		movePage("http://".$_SERVER['HTTP_HOST']."/{$ShopPath}{$Main}?channel=order_end&amp;order_num={$order_num}");
	break;
}
?>