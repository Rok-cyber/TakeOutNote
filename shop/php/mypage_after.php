...
$sql = "SELECT COUNT(*) FROM  mall_goods_point WHERE id='{$my_id}'";
...
$tpl->define("main","{$skin}/mypage_after.html");
$tpl->scan_area("main");

/**************************** GOODS LIST**************************/

if($TOTAL>0) {
	$sql = "SELECT * FROM mall_goods_point WHERE id = '{$my_id}' ORDER BY uid desc LIMIT {$Pstart},{$record_num}";
	$mysql->query($sql);
	$NUM = $TONUM;
	...
	if($TOTAL > $record_num){
		...
		$pg->addQueryString($Main."?channel=after"); 
