...
if(!$ord_id = $mysql->get_one($sql)) exit;
...
$tpl->parse("main");
//$tpl->tprint("main");
$MAIL_COMMENT = $tpl->tprint("main","1");
$tpl->close();
...
if($info['send_ok']=='N') {
	...
	if($mail_img[4]=='1') {
		...
		$MAIL_COMMENT = stripslashes($mail_img[5]).$MAIL_COMMENT;
		$MAIL_COMMENT = str_replace("../skin/", "{$URL}skin/",$MAIL_COMMENT); 
		$MAIL_COMMENT = str_replace("{$Main}", "{$URL}{$Main}",$MAIL_COMMENT); 
		$MAIL_COMMENT = str_replace("image/goods_img", "{$URL}image/goods_img",$MAIL_COMMENT);   		
		$MAIL_COMMENT = str_replace("{name}",$name,$MAIL_COMMENT);
		$MAIL_COMMENT = str_replace("{price}",str_replace("원","",$TOTAL3),$MAIL_COMMENT);
		include "mail_form.php";   //메일 양식 인클루드
		$mail_form = str_replace("{shopName}",$basic[1],$mail_form);
		pmallMailSend($info['email'], "감사합니다. {$basic[1]}에서 정상적으로 주문이 접수 되었습니다.", $mail_form);	
		//pmallMailSend($basic[10], "{$basic[1]}에서 주문이 접수 되었습니다.", $mail_form);	
	}
	############ 상품구매 메일 보내기 ############
	...
	if($info['hphone1']) {
		if($info['pay_type']=='B') {
			...
			$code_arr['price'] = str_replace("원","",$TOTAL3);
			$code_arr['account'] = $BANK_NAME;
			pmallSmsAutoSend($info['hphone1'],"order",$code_arr);
			pmallSmsAutoSend($info['hphone1'],"bank",$code_arr);
		}
		else if($info['pay_type']=='C' && $info['pay_status']=='B') {
			$code_arr = Array();
			$code_arr['name'] = $name;
			$code_arr['number'] = $order_num;
			$code_arr['price'] = str_replace("원","",$TOTAL3);
	...
	$sql = "UPDATE mall_order_info SET send_ok='Y' WHERE order_num='{$order_num}'";
	$mysql->query($sql);
}
