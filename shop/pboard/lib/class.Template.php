...
*/
	...
	function define_paging2($var,$parent) {
		$buffer = $this->Src[$parent];
		$buff = explode("<!-- DYNAMIC @$var@ -->", $buffer);
		if(count($buff) == 3) {
			$this->Src[$var] = "{{PAGING}}";
	...
	function getPgstring2() {
		return $this->pg_string2;
	}
...
*/
	...
	function scan_area($parent) {
		...
		while(is_long($pos = strpos($buffer, "<!-- DYNAMIC @", $pos_offset))) { 
			...
			else if($child=="define_pg2") {
				$this->define_paging2($child,$parent);
			}
