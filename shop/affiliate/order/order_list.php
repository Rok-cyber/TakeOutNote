...
if($total_record > 0) {
	...
	/*********************************** QUERY **********************************/
    if($join_use=='Y') {
		$query = "SELECT a.p_cate, a.p_number, a.order_num, a.p_name, a.p_qty, a.p_price, a.op_price, a.sale_price, a.order_status as status, a.carr_info FROM mall_order_goods as a, mall_order_info as b WHERE a.order_num=b.order_num && a.order_num>='{$Start}' && a.order_num<='{$End}' {$where} ORDER BY a.uid DESC";
	}
	else{
		$query = "SELECT a.p_cate, a.p_number, a.order_num, a.p_name, a.p_qty, a.p_price, a.op_price, a.sale_price, a.order_status as status, a.carr_info FROM mall_order_goods as a WHERE a.order_num>='{$Start}' && a.order_num<='{$End}' {$where} ORDER BY a.uid DESC";
	}
    $mysql->query($query);		
	/*********************************** QUERY  ***********************************/
	...
	while ($row=$mysql->fetch_array()){
		...
		if($tmp_order_num!=$row['order_num']) {
			$sql = "SELECT uid, order_num, id, name1, pay_type, pay_status, order_status, use_cupon, a_commi, use_reserve, carriage, pay_total, signdate FROM mall_order_info WHERE order_num='{$row['order_num']}'";
			...
			$tmps = stripslashes($row2['name1']);
			$LIST3 = substr($tmps,0,2);
			for($i=1,$cnts=strlen($tmps);$i<$cnts;$i+=2){
				$LIST3 .="*";	
			}
			$tmps = stripslashes($row2['id']);
			$LIST4 = substr($tmps,0,2);
			for($i=1,$cnts=strlen($tmps);$i<$cnts;$i++){
				$LIST4 .="*";	
			}
			$tmps = '';
			...
			$LIST10 = number_format($row2['use_cupon'],$ckFloatCnt);
			if($row2['a_commi']>0) {
				$LIST12 = number_format((($row2['pay_total'] + $row2['use_reserve'] - $row2['carriage'])*$row2['a_commi'])/100,$ckFloatCnt)."원<br />({$row2['a_commi']}%)";
			}
			else $LIST12 = 0;
		}		
