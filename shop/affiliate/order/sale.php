...
$tpl->scan_area("main");
$tmps = "";
$img_arr = Array("","year","month","day","detail");
for($i=1;$i<5;$i++){
...
if($mode=='detail') {
	...
	$TSALE1 = $TSALE2 = $TSALE3 = $TSALE4 = $TSALE5 = $TSALE6 = 0;
	...
	while($data = $mysql->fetch_array()){
		...
		$AFFILI  = "<a href='sale.php?mode=day&affiliates={$data['id']}&year={$year}&month={$month}' title='일별통계보기'>{$data['id']}</a>";
		$SALE1 = "<a href='order_list.php?sdate1={$i2}&sdate2={$i2}&affiliates={$data['id']}'><font class=eng>{$SALE1}</font></a>";
	...
	for($i=$f_s;$i<=$f_e;$i++){
		...
		switch($mode) {
			case "year" :
			    ...
			    $SALE1 = "<a href='order_list.php?sdate1={$i}&sdate2={$i}'><font class=eng>{$SALE1}</font></a>";
			break;
			case "month" :
				$DAYS  = "<a href='sale.php?mode=day&year={$year}&month={$i}' title='일별통계보기'>{$DAYS}</a>";
				$SALE1 = "<a href='order_list.php?sdate1={$i2}&sdate2={$i2}'><font class=eng>{$SALE1}</font></a>";
			break;
			case "day" :
				$DAYS  = "<a href='sale.php?mode=detail&sdate1={$i2}&sdate2={$i2}' title='상세내역보기'>{$DAYS}</a>";
				$SALE1 = "<a href='order_list.php?sdate1={$i2}&sdate2={$i2}{$addstring}'><font class=eng>{$SALE1}</font></a>";				
			break;
		}	
		$tpl->parse("loop");
