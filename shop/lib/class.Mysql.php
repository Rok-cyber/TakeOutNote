...
*/
    ...
    function query2($query) {
	 ...
	 function query3($query) {
        ...
        $this->connect();
        $this->query   = $query;
        $this->res3 = mysql_query($this->query,$this->con) or $this->errorMsg("que");  
	 ...
	 function query4($query) {
        ...
        $this->connect();
        $this->query   = $query;
        $this->res4 = mysql_query($this->query,$this->con) or $this->errorMsg("que");  
...
*/
    ...
    {   
	  if(!$no) {	
		...
		if(!$this->res3) return false; return mysql_fetch_array($this->res3); 
	  } else if($no==4) {
		if(!$this->res4) return false; return mysql_fetch_array($this->res4); 
	  }
    }
    ...
    {   
	  if(!$no) {
		...
		if(!$this->res3) return false; return mysql_fetch_object($this->res3); 
	  } else if($no==3) {
		if(!$this->res4) return false; return mysql_fetch_object($this->res4); 
	  }
	}
...
} // End of if(!isset($__PREVIL_MYSQL__)) 