...
<?
    ...
    include "./cash/configure/site.conf";
    /* ============================================================================== */
...
$mysql = new mysqlClass();     
    ...
    /* = -------------------------------------------------------------------------- = */
				...
				$receipt_info = "거래번호 : {$cash_no}, 승인시간 : {$app_time}";
				$signdate = date("Y-m-d H:i:s",time());

				$sql = "UPDATE mall_order_cash SET trad_time = '{$trad_time}', receipt_no = '{$receipt_no}', receipt_info ='{$receipt_info}', receipt_status='{$reg_stat}', receipt_error='', status='B', status_date='{$signdate}' WHERE order_num='{$ordr_idxx}'";
				$mysql->connect();
    ...
    /* = -------------------------------------------------------------------------- = */
                if ( $bSucc == "false" )
                {
					...
					if(eregi("CASH_",$ordr_idxx)) {
						$sql = "UPDATE mall_order_cash SET order_num = '', receipt_error='{$res_msg}' WHERE order_num='{$ordr_idxx}' && status='A'";						
					}
					else  {
						$sql = "UPDATE mall_order_cash SET receipt_error='{$res_msg}' WHERE order_num='{$ordr_idxx}' && status='A'";					
					}
					$mysql->query($sql);
					echo "False";
					exit;
    ...
    /* = -------------------------------------------------------------------------- = */
        else if ( $req_tx == "mod" )
        {
            if ( $res_cd == "0000" )
            {
				...
				$sql = "UPDATE mall_order_cash SET receipt_no = '{$receipt_no}', receipt_info ='{$receipt_info}', receipt_status='{$reg_stat}', receipt_error='', status='C', status_date='{$signdate}' WHERE uid='{$uid}'";
				$mysql->query($sql);					
			}
    /* = -------------------------------------------------------------------------- = */
    ...
    /* = -------------------------------------------------------------------------- = */
            else
            {
				...
				$sql = "UPDATE mall_order_cash SET receipt_error='{$res_msg}' WHERE uid='{$uid}'";
				$mysql->query($sql);
				echo "False";
				exit;
            }
